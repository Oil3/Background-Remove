...
struct GIFEditorView: View {
    ...
    var body: some View {
        VStack {
         ...
         .fileImporter(isPresented: true, allowedContentTypes: [.image], allowsMultipleSelection: false) { result in
            switch result {
            case .success(let urls):
                let gifURL = urls[0]
                    // Ensure the selected file is a GIF (optional, based on your needs)
                if gifURL.pathExtension.lowercased() == "gif" {
                        // Use your GIFProcessor to load and handle the GIF
                    gifProcessor.loadGIF(url: gifURL)
                }
